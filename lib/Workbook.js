...
Workbook.prototype.output = function () {
    ...
    for (var i = 0; i < this.sheets.length; i++) {
        ...
        // Row nodes <row> must be sorted by row number to comply with MS Excel
        // Cell nodes <c> must be sorted by address to comply with MS Excel
        // TODO:
        // * determine the <row> attribute "span" and update it
       
        var sortSheetData = function (nodeA, nodeB) {
            var rowNumberA = parseInt(nodeA.get('r'));
            var rowNumberB = parseInt(nodeB.get('r'));
            if (rowNumberA === rowNumberB) return 0;
        ...
        var sortRowByColumn = function (nodeA, nodeB) {
            ...
            if (refA.row !== refB.row) {
                throw new Error('Requires uniform column attribute in child node <c>');
            }
        ...
        sheetDataNode._children.sort(sortSheetData);
